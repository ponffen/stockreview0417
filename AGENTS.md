...
- **社区他人主页**：`GET /api/community/users/:id/profile` 仅身份五字段；**收益 Tab** 首屏 profile + `GET /api/public/:id/home-bundle`（`redactPublicHomeBundle`）；**交易 Tab** `GET /api/public/:id/trades`；**分析 Tab** 仅 `GET /api/public/:id/analysis-bundle`（`getMetricsPublicAnalysisBundle` → `redactPublicAnalysisBundle` → `src/metrics/series-index-normalize.js` 金额指数归一化四位小数、无 `profitCny`/`nameCn`/顶层 `benchmark`，`series` 键 `stageRate`/`stageProfit`；换算法只改该模块）。分析 Tab UI：复用 `#route-analysis` + `paintAnalysisFromMetricsApi(publicTargetId)`，隐藏账户下拉。Vercel public 路由在 `api/index.js` 直连。他人收益 Tab：**按列索引**克隆 `#route-earning` th + `paintStockTableFromMetricsRows`（`PUBLIC_EARNING_VISIBLE_COL_INDICES`）。
