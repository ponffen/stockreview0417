...
- **Bundle API**：首页 `GET /api/metrics/home-bundle?accountScope=&stages=today,mtd,ytd,inception`；分析页 `GET /api/metrics/analysis-bundle?accountScope=&stage=&symbol=`（可选基准）。Vercel 在 `api/index.js` 与 home-bundle 同样独立直连（勿落 Express）；本地 `server.js` 同路径。分析页不再拉 `/api/trades`、`/api/cash-transfers`。社区公开：`/api/public/:targetId/metrics/analysis-bundle`。
