...
- **Bundle API**：首页 `GET /api/metrics/home-bundle`；分析页 `GET /api/metrics/analysis-bundle`（一次含收益序列、收益率序列、资产四曲线 `dailyAssets.series.{total_assets,market_value,cash,cash_ratio}`、排行、阶段收益）。响应根字段 `metricsArchitecture: "v3"`、`metricsTables: ["analysis_daily_snapshot","symbol_daily_pnl"]`，**无 `*Display` 字段**（展示由前端格式化）。切换分析页资产下拉只本地换图、不重新请求。Vercel `api/index.js` 直连；分析页不拉 `/api/trades`、`/api/cash-transfers`。
