...
- **首屏数据**：SPA 启动 `GET /api/settings`（设置 + 账户列表 + `dataVersion`/`rebuilding`/`frozenThrough`）；成交/银证用 `/api/trades`、`/api/cash-transfers` 按需加载。收益/资产/持仓/分析图统一走 Bundle API：`/api/metrics/home-bundle`、`/api/metrics/analysis-bundle`、`/api/metrics/stock-record-bundle`；社区公开页 `GET /api/public/:targetId/...` 同路径。全量重算：`npm run rebuild:metrics` 或 `DATABASE_URL=... node scripts/rebuild-all-metrics.js`（默认仅 `metrics_enabled=1` 用户；`--all-users` 扫全员）。Cron 运行记录：`cron_job_run` 表 + `GET /api/ops/cron-runs`（需 env `ADMIN_USER_IDS` 或 `ADMIN_PHONES`）。**EOD 定时**：阿里云 FC `stockreview-eod-cron-trigger`（北京时间 08:00 周二～六，`CRON_TZ=Asia/Shanghai`）POST `https://www.higcc.com/api/cron/freeze-eod`（`x-cron-secret`）；部署 `scripts/setup-and-deploy-aliyun-eod-cron.sh`；Vercel `vercel.json` 不再配 `crons`。
- **指标计算开关**：`users.metrics_enabled`（默认 `0`，无自动激活）。仅 `1` 参与 EOD 冻结、catch-up、默认全量重算、`daily-close-sync` 全局日 K；写路径 freeze 在 `0` 时跳过（`metrics-disabled`）。显式 `userIds` 的 cron/脚本仍可重算指定用户。运维：`UPDATE users SET metrics_enabled = 1 WHERE ...` 后对该用户手动 freeze/rebuild。
- **公开到社区**：`users.community_public`（默认 `0`）。仅 `1` 时社区可见持仓收益/占比（不公开金额）。新注册用户默认关；运维手改库或用户在「我的」开关开启。
- **Vercel + Neon**：`VERCEL=1` 时 `src/db.js` 的 `q()` 每次查询独立连接；可用 `VERCEL_DB_SLOT_MAX`（默认 6）限制单实例并发连接，减轻 pooler 排队。
- **指标 v3**：EOD / cron 走 `metrics/freeze-v3`（`analysis_daily_snapshot` 全历史 + `symbol_daily_pnl` 仅冻结日一行）。启动时 `ensurePerformanceSchemaV2()` 会 DROP 已废弃表 `account_home_summary`、`symbol_home_summary`、`daily_returns`、`performance_series_cache`。
