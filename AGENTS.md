...
- **首页汇总**：`rebuildHomeSummaryForUser`（EOD 冻结后 / `npm run rebuild:home-summary`）会写入 **all + 各子账户** 的 `account_home_summary` / `symbol_home_summary`；子账户 `frozen_through` 须与 analysis 对齐，否则 `home-bundle` 持仓会偏旧。
