...
- **Bundle API（v2 终版）**：`GET /api/metrics/home-bundle`（`meta` + `returns.stages` + `assets` + `holdings.rows`）；`GET /api/metrics/analysis-bundle`（`meta` + 扁平 `returns` + `assets` + `series.{dailyProfit,dailyTwr,totalAssets,marketValue,cash,cashRatio}` + `stockRank`）。对外均为已格式化字符串，无 `*Display`、无 `*Num`；读库→算→直出 JSON（`bundle-payload` 仅加 `metricsArchitecture`）。Query 支持 `account_id`（兼容 `accountScope`）。持仓表排序由前端解析字符串（规则同原 CNY/涨跌幅逻辑）。分析页不拉 `/api/trades`、`/api/cash-transfers`、`/api/surface/account-kpis`。
- **社区他人主页**：`GET /api/community/users/:id/profile` 仅身份五字段；**收益 Tab** 首屏只拉 profile + `GET /api/public/:id/home-bundle`（兼容 `/metrics/home-bundle`，`redactPublicHomeBundle` 脱敏）；**交易 Tab** 再拉 `GET /api/public/:id/trades`；**分析 Tab** 再拉 `GET /api/public/:id/analysis-ui-prefs`（对方分析控件偏好，与 home-bundle 无关）及 `/api/public/:id/metrics/analysis-bundle` 等。Vercel 上上述 public 路由在 `api/index.js` 直连，勿落 Express 冷启动。他人收益 Tab UI：**按列索引**从 `#route-earning` 筛 th + `paintStockTableFromMetricsRows`（`PUBLIC_EARNING_VISIBLE_COL_INDICES` 物理 10 列，与私人同列规则）；KPI 仅现金/股票占比；四格收益率用 `profit-main`。
