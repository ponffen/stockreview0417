...
- **社区他人主页**：`GET /api/community/users/:id/profile` 仅身份五字段（`userId`/`displayName`/`following`/`mutual`/`isSelf`）；收益 Tab 用 `GET /api/public/:id/metrics/home-bundle`（同 private home-bundle，服务端 `redactPublicHomeBundle` 脱敏金额）；成交 `GET /api/public/:id/trades`；分析控件种子 `GET /api/public/:id/analysis-ui-prefs`；分析图仍走 `/api/public/:id/metrics/analysis-bundle` 等。他人收益 Tab 不随行情刷新重绘。
