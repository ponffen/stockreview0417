...
- **Bundle API**：首页 `GET /api/metrics/home-bundle?accountScope=&stages=today,mtd,ytd,inception`；分析页 `GET /api/metrics/analysis-bundle?accountScope=&stage=&symbol=`（可选基准）。Vercel 在 `api/index.js` 直连；本地 `server.js` 同路径。社区公开：`/api/public/:targetId/metrics/analysis-bundle`。
