...
- **Bundle API**：首页 `GET /api/metrics/home-bundle`；分析页 `GET /api/metrics/analysis-bundle`（一次含收益/收益率序列、资产四曲线 `dailyAssets.series.*`、排行、阶段收益）。展示值为服务端已格式化的字符串（如 `profit`、`ratePct`、`totalAssets`、`todayProfit` / `todayProfitCny`），**无 `*Display` 字段名**；数值字段（`profitCny`、`rate`、`*Num`）供着色/排序/图表。响应根字段 `metricsArchitecture: "v3"`、`metricsTables: ["analysis_daily_snapshot","symbol_daily_pnl"]`。分析页切换资产下拉只本地换图；不拉 `/api/trades`、`/api/cash-transfers`、`/api/surface/account-kpis`。
